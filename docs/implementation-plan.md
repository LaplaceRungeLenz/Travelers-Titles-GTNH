...
- [ ] M1: wrapper/build configuration, test dependencies and client-only entrypoint. `gradlew test build` must produce a reobfuscated Java 8 jar.
- [ ] M2: `core/Location`, `core/TitleController`, `core/Animation`. Test initially missing implementations, then verify debounce, latest candidate, same-name dimensions, reset, zero duration.
- [ ] M3: `core/RuleEngine`, `core/TitleStyle`, `client/TitleResources`, `client/ClientConfig`, `client/ConfigGuiFactory`, `client/HudRenderer`. Test layering, invalid fields, exact/glob matches and resource fallback. Verify rendering in-game.
- [ ] M4: `client/LocationResolver`, `client/SpaceCompat`, `client/TitleCommand`. Match runtime provider/body rather than configured numeric IDs; enumerate runtime biome/body directories and test absent GC.
- [ ] M5: `scripts/` isolated test setup, runtime probes and screenshots. Run automated suite plus target instance. Record pass/fail/not tested separately.
